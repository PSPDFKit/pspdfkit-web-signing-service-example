...
Object.defineProperty(exports, "EXTENSION", ({
  enumerable: true,
  get: function () {
    return src_SnapshotResolver.EXTENSION;
  }
}));
Object.defineProperty(exports, "SnapshotState", ({
  enumerable: true,
  get: function () {
    return src_State.default;
  }
}));
Object.defineProperty(exports, "addSerializer", ({
  enumerable: true,
  get: function () {
    return src_plugins.addSerializer;
  }
}));
Object.defineProperty(exports, "buildSnapshotResolver", ({
  enumerable: true,
  get: function () {
    return src_SnapshotResolver.buildSnapshotResolver;
  }
}));
...
Object.defineProperty(exports, "getSerializers", ({
  enumerable: true,
  get: function () {
    return src_plugins.getSerializers;
  }
}));
Object.defineProperty(exports, "isSnapshotPath", ({
  enumerable: true,
  get: function () {
    return src_SnapshotResolver.isSnapshotPath;
  }
}));
...
var src_util = require("util");
var src_fs = src_interopRequireWildcard(require("graceful-fs"));
var src_snapshotUtils = require("@jest/snapshot-utils");
var src_jestMatcherUtils = require("jest-matcher-utils");
var src_SnapshotResolver = __webpack_require__("./src/SnapshotResolver.ts");
var src_printSnapshot = __webpack_require__("./src/printSnapshot.ts");
var src_utils = __webpack_require__("./src/utils.ts");
var src_plugins = __webpack_require__("./src/plugins.ts");
var src_State = src_interopRequireDefault(__webpack_require__("./src/State.ts"));
function src_interopRequireDefault(e) { return e && e.__esModule ? e : { default: e }; }
function src_getRequireWildcardCache(e) { if ("function" != typeof WeakMap) return null; var r = new WeakMap(), t = new WeakMap(); return (src_getRequireWildcardCache = function (e) { return e ? t : r; })(e); }
function src_interopRequireWildcard(e, r) { if (!r && e && e.__esModule) return e; if (null === e || "object" != typeof e && "function" != typeof e) return { default: e }; var t = src_getRequireWildcardCache(r); if (t && t.has(e)) return t.get(e); var n = { __proto__: null }, a = Object.defineProperty && Object.getOwnPropertyDescriptor; for (var u in e) if ("default" !== u && {}.hasOwnProperty.call(e, u)) { var i = a ? Object.getOwnPropertyDescriptor(e, u) : null; i && (i.get || i.set) ? Object.defineProperty(n, u, i) : n[u] = e[u]; } return n.default = e, t && t.set(e, n), n; }
var src_Symbol = globalThis['jest-symbol-do-not-touch'] || globalThis.Symbol;
var src_Symbol = globalThis['jest-symbol-do-not-touch'] || globalThis.Symbol;
var src_jestExistsFile = globalThis[src_Symbol.for('jest-native-exists-file')] || src_fs.existsSync;
/**
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
const src_DID_NOT_THROW = 'Received function did not throw'; // same as toThrow
const src_NOT_SNAPSHOT_MATCHERS = `Snapshot matchers cannot be used with ${(0, src_jestMatcherUtils.BOLD_WEIGHT)('not')}`;
const src_INDENTATION_REGEX = /^([^\S\n]*)\S/m;

// Display name in report when matcher fails same as in snapshot file,
// but with optional hint argument in bold weight.
const src_printSnapshotName = (concatenatedBlockNames = '', hint = '', count) => {
  const hasNames = concatenatedBlockNames.length > 0;
  const hasHint = hint.length > 0;
  return `Snapshot name: \`${hasNames ? (0, src_snapshotUtils.escapeBacktickString)(concatenatedBlockNames) : ''}${hasNames && hasHint ? ': ' : ''}${hasHint ? (0, src_jestMatcherUtils.BOLD_WEIGHT)((0, src_snapshotUtils.escapeBacktickString)(hint)) : ''} ${count}\``;
};
function src_stripAddedIndentation(inlineSnapshot) {
  // Find indentation if exists.
  const match = inlineSnapshot.match(src_INDENTATION_REGEX);
  if (!match || !match[1]) {
    // No indentation.
...
const src_fileExists = (filePath, fileSystem) => fileSystem.exists(filePath) || src_jestExistsFile(filePath);
const src_cleanup = (fileSystem, update, snapshotResolver, testPathIgnorePatterns) => {
  const pattern = `\\.${src_SnapshotResolver.EXTENSION}$`;
  ...
  const list = files.filter(snapshotFile => {
    ...
    if (!src_fileExists(testPath, fileSystem)) {
      if (update === 'all') {
        src_fs.unlinkSync(snapshotFile);
      }
      return true;
    }
    return false;
...
exports.cleanup = src_cleanup;
const src_toMatchSnapshot = function (received, propertiesOrHint, hint) {
  ...
  if (length === 2 && typeof propertiesOrHint === 'string') {
    ...
    if (typeof propertiesOrHint !== 'object' || propertiesOrHint === null) {
      ...
      let printedWithType = (0, src_jestMatcherUtils.printWithType)('Expected properties', propertiesOrHint, src_printSnapshot.printExpected);
      if (length === 3) {
        options.secondArgument = 'hint';
        options.secondArgumentColor = src_jestMatcherUtils.BOLD_WEIGHT;
        if (propertiesOrHint == null) {
          printedWithType += "\n\nTo provide a hint without properties: toMatchSnapshot('hint')";
        }
      }
      throw new Error((0, src_jestMatcherUtils.matcherErrorMessage)((0, src_jestMatcherUtils.matcherHint)(matcherName, undefined, src_printSnapshot.PROPERTIES_ARG, options), `Expected ${(0, src_jestMatcherUtils.EXPECTED_COLOR)('properties')} must be an object`, printedWithType));
    }
...
exports.toMatchSnapshot = src_toMatchSnapshot;
const src_toMatchInlineSnapshot = function (received, propertiesOrSnapshot, inlineSnapshot) {
  ...
  if (length === 2 && typeof propertiesOrSnapshot === 'string') {
    ...
    if (length === 3) {
      options.secondArgument = src_printSnapshot.SNAPSHOT_ARG;
      options.secondArgumentColor = src_printSnapshot.noColor;
    }
    if (typeof propertiesOrSnapshot !== 'object' || propertiesOrSnapshot === null) {
      throw new Error((0, src_jestMatcherUtils.matcherErrorMessage)((0, src_jestMatcherUtils.matcherHint)(matcherName, undefined, src_printSnapshot.PROPERTIES_ARG, options), `Expected ${(0, src_jestMatcherUtils.EXPECTED_COLOR)('properties')} must be an object`, (0, src_jestMatcherUtils.printWithType)('Expected properties', propertiesOrSnapshot, src_printSnapshot.printExpected)));
    }
    if (length === 3 && typeof inlineSnapshot !== 'string') {
      throw new Error((0, src_jestMatcherUtils.matcherErrorMessage)((0, src_jestMatcherUtils.matcherHint)(matcherName, undefined, src_printSnapshot.PROPERTIES_ARG, options), 'Inline snapshot must be a string', (0, src_jestMatcherUtils.printWithType)('Inline snapshot', inlineSnapshot, src_utils.serialize)));
    }
    properties = propertiesOrSnapshot;
  }
  return src_toMatchSnapshot_0({
    context: this,
    inlineSnapshot: inlineSnapshot === undefined ? undefined : src_stripAddedIndentation(inlineSnapshot),
    isInline: true,
    matcherName,
    properties,
    received
...
exports.toMatchInlineSnapshot = src_toMatchInlineSnapshot;
const src_toMatchSnapshot_0 = config => {
  const {
    context,
    hint,
    inlineSnapshot,
  ...
  if (isNot) {
    throw new Error((0, src_jestMatcherUtils.matcherErrorMessage)((0, src_printSnapshot.matcherHintFromConfig)(config, false), src_NOT_SNAPSHOT_MATCHERS));
  }
  if (snapshotState == null) {
    ...
    throw new Error(`${(0, src_printSnapshot.matcherHintFromConfig)(config, false)}\n\n` + 'Snapshot state must be initialized' + `\n\n${(0, src_jestMatcherUtils.printWithType)('Snapshot state', snapshotState, src_jestMatcherUtils.stringify)}`);
  }
  const fullTestName = currentTestName && hint ? `${currentTestName}: ${hint}` : currentTestName || ''; // future BREAKING change: || hint

  if (typeof properties === 'object') {
    if (typeof received !== 'object' || received === null) {
      throw new Error((0, src_jestMatcherUtils.matcherErrorMessage)((0, src_printSnapshot.matcherHintFromConfig)(config, false), `${(0, src_jestMatcherUtils.RECEIVED_COLOR)('received')} value must be an object when the matcher has ${(0, src_jestMatcherUtils.EXPECTED_COLOR)('properties')}`, (0, src_jestMatcherUtils.printWithType)('Received', received, src_printSnapshot.printReceived)));
    }
    const propertyPass = context.equals(received, properties, [context.utils.iterableEquality, context.utils.subsetEquality]);
    if (propertyPass) {
      received = (0, src_utils.deepMerge)(received, properties);
    } else {
      const key = snapshotState.fail(fullTestName, received);
      const matched = /(\d+)$/.exec(key);
      const count = matched === null ? 1 : Number(matched[1]);
      const message = () => `${(0, src_printSnapshot.matcherHintFromConfig)(config, false)}\n\n${src_printSnapshotName(currentTestName, hint, count)}\n\n${(0, src_printSnapshot.printPropertiesAndReceived)(properties, received, snapshotState.expand)}`;
      return {
        message,
        name: matcherName,
        pass: false
  ...
  const message = expected === undefined ? () => `${(0, src_printSnapshot.matcherHintFromConfig)(config, true)}\n\n${src_printSnapshotName(currentTestName, hint, count)}\n\n` + `New snapshot was ${(0, src_jestMatcherUtils.BOLD_WEIGHT)('not written')}. The update flag ` + 'must be explicitly passed to write a new snapshot.\n\n' + 'This is likely because this test is run in a continuous integration ' + '(CI) environment in which snapshots are not written by default.\n\n' + `Received:${actual.includes('\n') ? '\n' : ' '}${(0, src_printSnapshot.bReceivedColor)(actual)}` : () => `${(0, src_printSnapshot.matcherHintFromConfig)(config, true)}\n\n${src_printSnapshotName(currentTestName, hint, count)}\n\n${(0, src_printSnapshot.printSnapshotAndReceived)(expected, actual, received, snapshotState.expand, snapshotState.snapshotFormat)}`;
...
const src_toThrowErrorMatchingSnapshot = function (received, hint, fromPromise) {
  const matcherName = 'toThrowErrorMatchingSnapshot';

  // Future breaking change: Snapshot hint must be a string
  // if (hint !== undefined && typeof hint !== string) {}

  return src_toThrowErrorMatchingSnapshot_0({
    context: this,
    hint,
    isInline: false,
    matcherName,
...
exports.toThrowErrorMatchingSnapshot = src_toThrowErrorMatchingSnapshot;
const src_toThrowErrorMatchingInlineSnapshot = function (received, inlineSnapshot, fromPromise) {
  const matcherName = 'toThrowErrorMatchingInlineSnapshot';
  if (inlineSnapshot !== undefined && typeof inlineSnapshot !== 'string') {
    const options = {
      expectedColor: src_printSnapshot.noColor,
      isNot: this.isNot,
      promise: this.promise
    };
    throw new Error((0, src_jestMatcherUtils.matcherErrorMessage)((0, src_jestMatcherUtils.matcherHint)(matcherName, undefined, src_printSnapshot.SNAPSHOT_ARG, options), 'Inline snapshot must be a string', (0, src_jestMatcherUtils.printWithType)('Inline snapshot', inlineSnapshot, src_utils.serialize)));
  }
  return src_toThrowErrorMatchingSnapshot_0({
    context: this,
    inlineSnapshot: inlineSnapshot === undefined ? undefined : src_stripAddedIndentation(inlineSnapshot),
...
exports.toThrowErrorMatchingInlineSnapshot = src_toThrowErrorMatchingInlineSnapshot;
const src_toThrowErrorMatchingSnapshot_0 = (config, fromPromise) => {
  const {
    context,
    hint,
    inlineSnapshot,
  ...
  if (!fromPromise) {
    if (typeof received !== 'function') {
      ...
      throw new Error((0, src_jestMatcherUtils.matcherErrorMessage)((0, src_jestMatcherUtils.matcherHint)(matcherName, undefined, '', options), `${(0, src_jestMatcherUtils.RECEIVED_COLOR)('received')} value must be a function`, (0, src_jestMatcherUtils.printWithType)('Received', received, src_printSnapshot.printReceived)));
    }
  }
  if (isNot) {
    throw new Error((0, src_jestMatcherUtils.matcherErrorMessage)((0, src_printSnapshot.matcherHintFromConfig)(config, false), src_NOT_SNAPSHOT_MATCHERS));
  ...
  while ('cause' in error) {
    error = error.cause;
    if (src_util.types.isNativeError(error) || error instanceof Error) {
      message += `\nCause: ${error.message}`;
    } else {
