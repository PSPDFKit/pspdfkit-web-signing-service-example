...
Object.defineProperty(exports, "DIFF_DELETE", ({
  enumerable: true,
  get: function () {
    return src_cleanupSemantic.DIFF_DELETE;
  }
}));
Object.defineProperty(exports, "DIFF_EQUAL", ({
  enumerable: true,
  get: function () {
    return src_cleanupSemantic.DIFF_EQUAL;
  }
}));
Object.defineProperty(exports, "DIFF_INSERT", ({
  enumerable: true,
  get: function () {
    return src_cleanupSemantic.DIFF_INSERT;
  }
}));
Object.defineProperty(exports, "Diff", ({
  enumerable: true,
  get: function () {
    return src_cleanupSemantic.Diff;
  }
}));
exports.diff = src_diff;
Object.defineProperty(exports, "diffLinesRaw", ({
  enumerable: true,
  get: function () {
    return src_diffLines.diffLinesRaw;
  }
}));
Object.defineProperty(exports, "diffLinesUnified", ({
  enumerable: true,
  get: function () {
    return src_diffLines.diffLinesUnified;
  }
}));
Object.defineProperty(exports, "diffLinesUnified2", ({
  enumerable: true,
  get: function () {
    return src_diffLines.diffLinesUnified2;
  }
}));
Object.defineProperty(exports, "diffStringsRaw", ({
  enumerable: true,
  get: function () {
    return src_printDiffs.diffStringsRaw;
  }
}));
Object.defineProperty(exports, "diffStringsUnified", ({
  enumerable: true,
  get: function () {
    return src_printDiffs.diffStringsUnified;
  }
}));
var src_chalk = src_interopRequireDefault(require("chalk"));
var src_jestGetType = require("jest-get-type");
var src_prettyFormat = require("pretty-format");
var src_cleanupSemantic = __webpack_require__("./src/cleanupSemantic.ts");
var src_constants = __webpack_require__("./src/constants.ts");
var src_diffLines = __webpack_require__("./src/diffLines.ts");
var src_normalizeDiffOptions = __webpack_require__("./src/normalizeDiffOptions.ts");
var src_printDiffs = __webpack_require__("./src/printDiffs.ts");
 ...
 * LICENSE file in the root directory of this source tree.
 */
const src_getCommonMessage = (message, options) => {
  const {
    commonColor
  } = (0, src_normalizeDiffOptions.normalizeDiffOptions)(options);
  return commonColor(message);
};
const {
  AsymmetricMatcher: src_AsymmetricMatcher,
  DOMCollection: src_DOMCollection,
  DOMElement: src_DOMElement,
  Immutable: src_Immutable,
  ReactElement: src_ReactElement,
  ReactTestComponent: src_ReactTestComponent
} = src_prettyFormat.plugins;
const src_PLUGINS = [src_ReactTestComponent, src_ReactElement, src_DOMElement, src_DOMCollection, src_Immutable, src_AsymmetricMatcher];
const src_FORMAT_OPTIONS = {
  plugins: src_PLUGINS
};
const src_FALLBACK_FORMAT_OPTIONS = {
  callToJSON: false,
  maxDepth: 10,
  plugins: src_PLUGINS
};
...
function src_diff(a, b, options) {
  if (Object.is(a, b)) {
    return src_getCommonMessage(src_constants.NO_DIFF_MESSAGE, options);
  }
  const aType = (0, src_jestGetType.getType)(a);
  ...
  if (expectedType !== (0, src_jestGetType.getType)(b)) {
    return '  Comparing two different types of values.' + ` Expected ${src_chalk.default.green(expectedType)} but ` + `received ${src_chalk.default.red((0, src_jestGetType.getType)(b))}.`;
  }
  if (omitDifference) {
    return null;
  }
  switch (aType) {
    case 'string':
      return (0, src_diffLines.diffLinesUnified)(a.split('\n'), b.split('\n'), options);
    case 'boolean':
    case 'number':
      return src_comparePrimitive(a, b, options);
    case 'map':
      return src_compareObjects(src_sortMap(a), src_sortMap(b), options);
    case 'set':
      return src_compareObjects(src_sortSet(a), src_sortSet(b), options);
    default:
      return src_compareObjects(a, b, options);
  }
}
function src_comparePrimitive(a, b, options) {
  const aFormat = (0, src_prettyFormat.format)(a, src_FORMAT_OPTIONS);
  const bFormat = (0, src_prettyFormat.format)(b, src_FORMAT_OPTIONS);
  return aFormat === bFormat ? src_getCommonMessage(src_constants.NO_DIFF_MESSAGE, options) : (0, src_diffLines.diffLinesUnified)(aFormat.split('\n'), bFormat.split('\n'), options);
}
function src_sortMap(map) {
  return new Map([...map].sort());
}
function src_sortSet(set) {
  return new Set([...set].sort());
}
function src_compareObjects(a, b, options) {
  let difference;
  let hasThrown = false;
  try {
    const formatOptions = src_getFormatOptions(src_FORMAT_OPTIONS, options);
    difference = src_getObjectsDifference(a, b, formatOptions, options);
  } catch {
    hasThrown = true;
  }
  const noDiffMessage = src_getCommonMessage(src_constants.NO_DIFF_MESSAGE, options);
  // If the comparison yields no results, compare again but this time
  // without calling `toJSON`. It's also possible that toJSON might throw.
  if (difference === undefined || difference === noDiffMessage) {
    const formatOptions = src_getFormatOptions(src_FALLBACK_FORMAT_OPTIONS, options);
    difference = src_getObjectsDifference(a, b, formatOptions, options);
    if (difference !== noDiffMessage && !hasThrown) {
      difference = `${src_getCommonMessage(src_constants.SIMILAR_MESSAGE, options)}\n\n${difference}`;
    }
  }
  return difference;
}
function src_getFormatOptions(formatOptions, options) {
  const {
    compareKeys
  } = (0, src_normalizeDiffOptions.normalizeDiffOptions)(options);
  return {
...
function src_getObjectsDifference(a, b, formatOptions, options) {
  const formatOptionsZeroIndent = {
    ...formatOptions,
    indent: 0
  };
  const aCompare = (0, src_prettyFormat.format)(a, formatOptionsZeroIndent);
  const bCompare = (0, src_prettyFormat.format)(b, formatOptionsZeroIndent);
  if (aCompare === bCompare) {
    return src_getCommonMessage(src_constants.NO_DIFF_MESSAGE, options);
  } else {
    const aDisplay = (0, src_prettyFormat.format)(a, formatOptions);
    const bDisplay = (0, src_prettyFormat.format)(b, formatOptions);
    return (0, src_diffLines.diffLinesUnified2)(aDisplay.split('\n'), bDisplay.split('\n'), aCompare.split('\n'), bCompare.split('\n'), options);
  }
}
})();
