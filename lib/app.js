...
app.post("/sign", async (req, res) => {
  ...
  const signHandler = signToken.signMethod.getSignMethodHandler();
  ...
  try {
    ...
    if (action == "get_certificates") {
      const certificates = await signHandler.getCertificates();
      ...
      if (action == "sign_pkcs7") {
        ...
        const signedData = await signHandler.signPkcs7(
          documentContents,
        ...
        const signedData = await signHandler.signRaw(
          dataToBeSigned,
          req.body.hash_algorithm || "sha256"
        );
        sendSignedData(res, signedData);
